...
@implementation UIDevice (LXCategory)
...
static uint64_t _net_counter_get_by_type(net_interface_counter *counter, LXNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & LXNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & LXNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & LXNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & LXNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & LXNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & LXNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}
